...
def push_data(data, channel):
    channel.basic_publish(exchange='', routing_key='transform', body=str(data.transform))
    channel.basic_publish(exchange='', routing_key='accel', body=str(data.accelerometer))
    channel.basic_publish(exchange='', routing_key='gyro', body=str(data.gyroscope))
    channel.basic_publish(exchange='', routing_key='compass', body=str(data.compass))
...
channel.queue_declare(queue='transform')
channel.queue_declare(queue='accel')
channel.queue_declare(queue='gyro')
channel.queue_declare(queue='compass')
